...
- **Smart Query Syntax:** Space=AND, `-`=exclude, `num~num`=range search, `col:name`=column search, `/regex/`=advanced pattern, `"phrase"`=quoted phrase search
...
- **スマートクエリ構文:** スペース=AND、`-`=除外、`数字~数字`=範囲検索、`col:名前`=特定列検索、`/正規表現/`=高度なパターンマッチ、`"フレーズ"`=空白を含む検索語束ね
...
- **智慧查詢語法:** 空格=AND、`-`=排除、`數字~數字`=範圍查詢、`col:名稱`=特定欄位查詢、`/正規表示式/`=進階比對、`"片語"`=引號包覆空白詞彙
...
- **智能查询语法:** 空格=AND、`-`=排除、`数字~数字`=范围查询、`col:名称`=特定列查询、`/正则表达式/`=高级匹配、`"词组"`=引号包裹空格词汇
