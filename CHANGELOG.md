...
### 수정됨 (Fixed)
- **🚨 AND 조건이 초성 검색을 죽이는 현상** — `ㅎㄱㄷ 100~500` 등 초성 + 다른 조건 혼합 시, AND 후처리기가 원본 텍스트에서 "ㅎㄱㄷ"을 문자 그대로 찾아 실패하던 버그. `extractChosung`으로 초성 변환 후 매칭을 병행하여 초성 검색 결과가 AND 필터에서 살아남도록 수정.

### 알려진 한계 (Known Limitations)
- **열 모드 + 정규식 결합 불가** — `col:이름 /[가-힣]+/`는 정규식이 먼저 추출되어 열 모드와 결합되지 않음. 정규식은 항상 전체 데이터 대상으로 적용됨 (파서 설계 한계, AST 필요).

## [2.5.4] - 2026-02-25

### 수정됨 (Fixed)
- **🚨 동종 다중 조건 AND 붕괴** — `/apple/ /banana/`(정규식 2개), `100~200 400~500`(범위 2개), `col:A x col:B y`(열 2개) 등 동종 다중 조건에서 AND가 발동되지 않던 버그 수정. `queryTypeCount`(유형 수) → `totalConditions`(조건 총합)으로 전환.

### 개선됨 (Improved)
- **열 필터 키워드 하이라이트** — `col:이름 홍길동` 검색 시 '홍길동'에 노란색 하이라이트가 칠해지도록 columnFilters의 keyword도 하이라이트 대상에 포함.

## [2.5.3] - 2026-02-25

### 수정됨 (Fixed)
- **🚨 정규식 vocabulary 역참조 모순** — 토큰화 시 하이픈/공백/대문자가 제거되어 `/\d{3}-\d{4}/`, `/[A-Z]+/` 등이 절대 매칭되지 않던 결함 수정. 원본 셀 O(N) 풀스캔으로 복원 (상한 제거, V8에서 100만 셀 20~40ms).
- **🚨 AND 조건 대소문자 충돌** — `rowText`가 toLowerCase()되어 `/Apple/` 등 대문자 정규식이 실패하던 버그. 원본(`rowTextOriginal`)과 소문자(`rowTextLower`) 텍스트를 분리하여 각각 적절한 대상에 사용.
- **🚨 열 필터 .find() 단일 매칭** — "영문이름"이 "이름"보다 먼저 걸리는 버그. `.filter().some()`으로 전환하여 부분 일치하는 모든 열을 검사.
